<x-app-layout>
    <h1 class="text-2xl font-semibold">Input Tugas</h1>
    <div class="bg-white rounded-md shadow p-4 mt-5">
        Form input Tugas
    </div>

    <form action="{{ route('storeTugas') }}" method="POST" class="bg-white rounded-md shadow p-4 mt-5">
        @csrf
        <div>
...
>>>>>>> origin/Damario
        ...
        <div class="mt-3">
            <label for="judul" class="block mb-2 text-sm font-medium text-gray-900">Judul Tugas:</label>
            <input id="judul" name="judul" type="text" required placeholder="Judul tugas..."
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
        </div>
        <div class="mt-3">
            <label for="tugas" class="block mb-2 text-sm font-medium text-gray-900">Konten Tugas:</label>
            <textarea id="tugas" rows="8" name="tugas" required
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Konten tugas..."></textarea>
        </div>
        <div class="mt-3">
            <button type="submit"
    ...
    <div class="bg-white rounded-md shadow p-4 mt-5">
        @if(count($tugas) > 0)
        <h1 class="mb-5 text-xl font-semibold">List Tugas {{ auth()->user()->guru->tingkatan_sekolah }}</h1>
        <div class="grid grid-cols-4 gap-5">
            ...
            <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow flex flex-col justify-between">
                ...
                <div>
                    ...
                    <p class="mb-3 font-normal text-gray-700">{{ Illuminate\Support\Str::limit($data->tugas, 120) }}</p>
                </div>
                    ...
                    <div class="relative bg-white rounded-lg shadow">
                        ...
                        <div class="p-4 md:p-5 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-500">Apakah kamu yakin untuk menghapus materi
                                ini?</h3>
                            <form action="{{ route('deleteTugas', $data->id) }}" method="POST">
                                @csrf
                                @method('delete')
                                <button type="submit"
            ...
            <div id="editModal-{{ $data->id }}" tabindex="-1" aria-hidden="true"
                ...
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    <!-- Modal content -->
                    <form form action="{{ route('updateTugas', $data->id) }}" method="POST"
                        ...
                        <div class="p-4 md:p-5 space-y-4">
                            ...
                            <div class="mt-3">
                                <label for="tugas" class="block mb-2 text-sm font-medium text-gray-900">Konten
                                    Materi:</label>
                                <textarea id="tugas" rows="8" name="tugas" required
                                    class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ $data->tugas }}</textarea>
                            </div>
                    </form>
                    <!-- Modal footer -->
